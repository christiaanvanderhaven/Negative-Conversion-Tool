...
convertarg [loc] = do 
  ...
  P.putStr "See UI to pick base...\n" 
    >> return (ChooseBase 0.25 (0, 0) (imageWidth negative, imageHeight negative) (fromImageRGB8 $ toImageRGB8 negative) negative (0, 0) (0, 0))
...
convertarg (loc:["-A"]) = do 
  ...
  print (sampleBase negative) >> P.putStr "Finds automatic base colour\n" 
    ...
    >> return (Result (fromImageRGB8 $ toImageRGB8 $ imageConvert negative (sampleBase negative)) 0.25 (0, 0))
...
-- Loads a file
convertFromFile :: String -> IO (Image PixelRGB16)
convertFromFile location = do
  dimage <- readImage location
  case dimage of
    Left err-> error "Image could not be read"
    Right dynamicImage -> return (convertRGB16 dynamicImage)

-- Saves to a file
saveConverted :: Image PixelRGB16 -> String -> IO ()
